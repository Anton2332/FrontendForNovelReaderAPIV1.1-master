...
export function fetchPosts(bookId: number, page: number) {
    return async (dispatch: Dispatch<postsActions>) => {
        try {
            dispatch(setIsPostsLoadingAction(true))

            const response = await postsService.getPosts(bookId, page)

            dispatch(setPostsAction(response.data.results as IPost[]))
...
export function postNewPost(body: string,userId: number, bookId: number) {
    return async (dispatch: Dispatch<postsActions>) => {
        try {
            const response = await postsService.postPost(body, userId, bookId)
